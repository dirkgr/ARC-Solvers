...
run_name=default
if [ -z $model_dir ] ; then
  echo "USAGE: ./scripts/evaluate_solver_with_gold.sh question_file.jsonl model_dir/"
  exit 1
fi
...
input_file_with_facts=${input_file_prefix}_with_facts_${run_name}.jsonl
...
if [ ! -f ${input_file_with_facts} ]; then
  python arc_solvers/processing/add_gold_retrieved_text.py \
    ${input_file} \
    ${input_file_with_facts}.$$
  mv ${input_file_with_facts}.$$ ${input_file_with_facts}
fi
